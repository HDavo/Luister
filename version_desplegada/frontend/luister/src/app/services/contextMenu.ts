...
export class ContexMenu {
    ...
    justFollow(){
        ...
        if(userid){
            ...
            const lookupkey = this.id.split(':')[1];
            let artist:any = {
                name: '',
                lookupkey: lookupkey,
                userid: userid,
            ...
            const getthem: {[key:string]:Function} = {
                sfy: ()=> {
                    this.fromSpoify.getArtist(lookupkey)
                    .then((response:any)=> response.subscribe((data:any)=>{
                        artist.name = data.name;
                        artist.image = data.images[0].url;
                        this.luister.follow(artist)
            ...
            alert('Debe iniciar sesion para realizar esta acción');
        }
        this.removeContexMenu();
    }
}