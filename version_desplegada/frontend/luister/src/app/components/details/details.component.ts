...
export class DetailsComponent extends SetData implements OnInit{
  ...
  ngOnInit(): void {
    console.log(this.likedFollowed)
  }
  ...
  like(event: any){
    ...
    if(userid){
      this.track.artists.forEach((artist:any, i:number)=>{
        art[i] = {"name": artist.name, "lookupkey": artist.id};
      })
    ...
    }else alert('Debes iniciar sesión para reealizar esta acción')
  }
  dislike(event: any){
    const userid = this.cookieService.get('userid');
    let title, lookupkey;
    ...
    }else alert('Debes iniciar sesión para realizar esta acción');
  }
  getLikedFollowed(){
    let data: any = {}
    const userid = this.cookieService.get('userid');
