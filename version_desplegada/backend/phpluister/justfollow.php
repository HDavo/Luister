<?php
    ...
    try {
        ...
        if($data){
            ...
            $prepQ = $conection->prepare("SELECT id FROM followedartists WHERE follower = :follower AND name = :name");
            ...
            }else echo json_encode(['status'=>406, 'message'=> 'Ya se encuentra siguiendo a '.$name]);

        }else die(json_encode(['status'=>400, 'message'=> 'Sin datos']));
